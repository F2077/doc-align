...
**Architecture:** Command-only. `/doc-align:align` is the sole entry point. No hooks.
...
├── commands/align.md              /doc-align:align command (sole entry point)
└── skills/align/SKILL.md        Core analysis skill (7-step workflow)
```
...
- [x] **Core skill (SKILL.md)** — 7-step AI workflow:
  ...
  5. Generate report
  6. User confirmation via AskUserQuestion (recommended, required by default)
  7. Execute surgical updates
- [x] **Manual trigger command** — `/doc-align:align` with optional commit range
